...
import { getTheResident } from "../graphql/queries";
import { updateTheResident } from "../graphql/mutations";
const client = generateClient();
export default function TheResidentUpdateForm(props) {
  const {
    id: idProp,
    theResident: theResidentModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = theResidentRecord
      ? { ...initialValues, ...theResidentRecord }
      : initialValues;
  ...
  const [theResidentRecord, setTheResidentRecord] =
    React.useState(theResidentModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getTheResident.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getTheResident
        : theResidentModelProp;
      setTheResidentRecord(record);
    };
    queryData();
  }, [idProp, theResidentModelProp]);
  React.useEffect(resetStateValues, [theResidentRecord]);
  const validations = {
    Name: [],
    phoneNo: [],
    address: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateTheResident.replaceAll("__typename", ""),
            variables: {
              input: {
                id: theResidentRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "TheResidentUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || theResidentModelProp)}
